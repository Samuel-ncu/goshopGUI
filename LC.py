...
class OrderScraperApp(QWidget):
    ...
    def scrape_data(self):
        ...
        if not os.path.exists(products_file):
            QMessageBox.information(self, "提示",
                                    f"使用者 {self.user_combo.currentText()} 尚未建立產品目錄 (products_list.xlsx)",
                                    QMessageBox.Ok)
            return

        if not self.page:
            self.log("請先啟動瀏覽器並手動登入。")
            QMessageBox.warning(self, "警告", "請先啟動瀏覽器並手動登入。", QMessageBox.Ok)
            return
        ...
        try:
            ...
            df_original = pd.DataFrame(pending_orders + rest_orders, columns=columns)

            if not os.path.exists(lastorder_file) and not df_original.empty:
                first_order_code = df_original["Order Code"].iloc[0].strip()
            ...
            if os.path.exists(products_file):
                ...
                df_products = pd.DataFrame(columns=["Name", "url"])

            # 新增 LINK 欄位
            link_list = []
            for idx, row in merged_df.iterrows():
                prod_name = row["Product Name"]
                match = df_products[df_products["Name"] == prod_name]
                if not match.empty:
                    url = match.iloc[0]["url"]
                    if pd.isna(url) or url == "":
                        link = ""
                    else:
                        link = f'=HYPERLINK("{url}", "點我")'
                else:
                    link = ""
                link_list.append(link)
            merged_df["LINK"] = link_list

            # 儲存 Excel 檔案
            file_path = os.path.join(self.current_user_dir, f"goshop_orders_{datetime.now().strftime('%Y%m%d')}.xlsx")
    ...
    def scrape_data_by_order_range(self, start_order, end_order):
        ...
        try:
            ...
            columns = ["#", "Order Code", "Num. of Products", "Customer", "Amount", "Service charge",
                       "Final price", "Delivery Status", "Payment Status", "Product Info", "Options"]
            df_original = pd.DataFrame(all_data, columns=columns)
